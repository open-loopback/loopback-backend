...
import { feedbacks, sources, webhookDeliveries } from "../db/schema.js";
...
export const feedbackRoute = new Hono()
    ...
    const projectWebhooks = await db.query.webhooks.findMany({
        where: (webhooks, { and, eq }) => and(eq(webhooks.projectId, source[0].projectId), eq(webhooks.isActive, true)),
    });
    const relevantWebhooks = projectWebhooks.filter(wh => wh.sources && wh.sources.includes(source[0].id));
